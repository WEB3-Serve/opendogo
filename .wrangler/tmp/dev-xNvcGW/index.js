...
// .wrangler/tmp/bundle-i5B40G/checked-fetch.js
...
// .wrangler/tmp/bundle-i5B40G/strip-cf-connecting-ip-header.js
...
var HOTP = class {
  ...
  */
  static generate({ secret, algorithm = HOTP.defaults.algorithm, digits = HOTP.defaults.digits, counter = HOTP.defaults.counter, hmac: hmac2 = hmacDigest }) {
  ...
  */
  static validate({ token, secret, algorithm, digits = HOTP.defaults.digits, counter = HOTP.defaults.counter, window: window2 = HOTP.defaults.window, hmac: hmac2 = hmacDigest }) {
  ...
  * @param {(algorithm: string, key: Uint8Array, message: Uint8Array) => Uint8Array} [config.hmac] Custom HMAC function.
  */
  constructor({ issuer = HOTP.defaults.issuer, label = HOTP.defaults.label, issuerInLabel = HOTP.defaults.issuerInLabel, secret = new Secret(), algorithm = HOTP.defaults.algorithm, digits = HOTP.defaults.digits, counter = HOTP.defaults.counter, hmac: hmac2 } = {}) {
    this.issuer = issuer;
    this.label = label;
...
var TOTP = class {
  ...
  * @returns {string} Token.
  */
  static generate({ secret, algorithm, digits, period = TOTP.defaults.period, timestamp = Date.now(), hmac: hmac2 }) {
    return HOTP.generate({
      secret,
      algorithm,
      digits,
  ...
  * @returns {number|null} Token delta or null if it is not found in the search window, in which case it should be considered invalid.
  */
  static validate({ token, secret, algorithm, digits, period = TOTP.defaults.period, timestamp = Date.now(), window: window2, hmac: hmac2 }) {
    return HOTP.validate({
      token,
      secret,
      algorithm,
  ...
  * @param {(algorithm: string, key: Uint8Array, message: Uint8Array) => Uint8Array} [config.hmac] Custom HMAC function.
  */
  constructor({ issuer = TOTP.defaults.issuer, label = TOTP.defaults.label, issuerInLabel = TOTP.defaults.issuerInLabel, secret = new Secret(), algorithm = TOTP.defaults.algorithm, digits = TOTP.defaults.digits, period = TOTP.defaults.period, hmac: hmac2 } = {}) {
    this.issuer = issuer;
    this.label = label;
    this.issuerInLabel = issuerInLabel;
    this.secret = typeof secret === "string" ? Secret.fromBase32(secret) : secret;
...
async function createToken(payload, expiresIn) {
  ...
  const token = await new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(expiration).sign(secret);
  return token;
}
...
function errorResponse(message2, status = 400, details = null) {
  return jsonResponse({
    success: false,
    error: message2,
    details
...
function successResponse(data, message2 = "Success") {
  return jsonResponse({
    success: true,
    message: message2,
    data
...
async function handleCardsList(request) {
  try {
    ...
    let query = supabase.from("card_keys").select("*, card_rules(name), card_groups(name)", { count: "exact" }).order("created_at", { ascending: false });
    if (status !== null) {
...
__name(handleCardsList, "handleCardsList");
async function handleCardsRules(request) {
  const method = request.method;
  if (method === "GET") {
    try {
      const { data, error } = await supabase.from("card_rules").select("*").order("created_at", { ascending: false });
      if (error) {
  ...
  if (method === "POST") {
    try {
      ...
      const { data, error } = await supabase.from("card_rules").insert({
        name,
        valid_days,
        max_queries_per_day,
        allow_batch,
...
__name(handleCardsRules, "handleCardsRules");
...
__name(handleCardsGenerate, "handleCardsGenerate");
async function handleCardsGroups(request) {
  const method = request.method;
  if (method === "GET") {
    try {
      const { data, error } = await supabase.from("card_groups").select("*").order("created_at", { ascending: false });
      if (error) {
  ...
  if (method === "POST") {
    try {
      ...
      const { data, error } = await supabase.from("card_groups").insert({
        name,
        description,
        created_at: (/* @__PURE__ */ new Date()).toISOString()
      }).select().single();
...
__name(handleCardsGroups, "handleCardsGroups");
...
__name(handleUsersCreate, "handleUsersCreate");
async function handleUsersRegister(request) {
  try {
    ...
    const { data: verifyData, error: verifyError } = await supabase.from("verify_codes").select("id, email, code, expires_at, used").eq("email", email).eq("code", verify_code).eq("used", false).gt("expires_at", now).order("created_at", { ascending: false }).limit(1).single();
    if (verifyError || !verifyData) {
      return errorResponse("Invalid or expired verification code", 400);
    }
    await supabase.from("verify_codes").update({ used: true }).eq("id", verifyData.id);
    const { data: existingUser } = await supabase.from("users").select("id").eq("username", username).or(`email.eq.${email}`).single();
    if (existingUser) {
    ...
    const passwordHash = Array.from(new Uint8Array(exportedKey)).map((b) => b.toString(16).padStart(2, "0")).join("");
    const passwordSalt = Array.from(salt).map((b) => b.toString(16).padStart(2, "0")).join("");
    const { data, error } = await supabase.from("users").insert({
      username,
      email,
      ...
      query_count: 0,
      created_at: (/* @__PURE__ */ new Date()).toISOString()
    }).select("id, username, email, card_key, created_at").single();
    if (error) {
...
__name(handleUsersRegister, "handleUsersRegister");
async function handleUsersSendVerifyCode(request) {
  try {
    ...
    const { error } = await supabase.from("verify_codes").insert({
      email,
      code: verifyCode,
...
__name(handleUsersSendVerifyCode, "handleUsersSendVerifyCode");
async function handleUsersGroups(request) {
  const method = request.method;
  if (method === "GET") {
    try {
      const { data, error } = await supabase.from("user_groups").select("*").order("created_at", { ascending: false });
      if (error) {
...
__name(handleUsersGroups, "handleUsersGroups");
async function handleConfigSite(request) {
  ...
  if (method === "PUT") {
    try {
      const body = await request.json();
      const { data: current } = await supabase.from("site_config").select("id").single();
      let result;
      if (current) {
        ...
        const { data, error } = await supabase.from("site_config").insert({
          ...body,
          created_at: (/* @__PURE__ */ new Date()).toISOString(),
          updated_at: (/* @__PURE__ */ new Date()).toISOString()
        }).select().single();
...
__name(handleConfigSite, "handleConfigSite");
async function handleConfigAnnouncement(request) {
  ...
  if (method === "GET") {
    try {
      ...
      if (published === "true") {
        query = query.eq("published", true).lte("publish_at", (/* @__PURE__ */ new Date()).toISOString()).or(`expire_at.is.null,gte.expire_at.${(/* @__PURE__ */ new Date()).toISOString()}`);
      }
  ...
  if (method === "POST") {
    try {
      ...
      const { data, error } = await supabase.from("announcements").insert({
        ...
        type,
        published,
        publish_at: publish_at || (/* @__PURE__ */ new Date()).toISOString(),
        expire_at,
        created_at: (/* @__PURE__ */ new Date()).toISOString()
      }).select().single();
      if (error) {
...
__name(handleConfigAnnouncement, "handleConfigAnnouncement");
async function handleConfigSystem(request) {
  ...
  if (method === "PUT") {
    try {
      const body = await request.json();
      const { data: current } = await supabase.from("system_config").select("id").single();
      let result;
      if (current) {
        ...
        const { data, error } = await supabase.from("system_config").insert({
          ...body,
          created_at: (/* @__PURE__ */ new Date()).toISOString(),
          updated_at: (/* @__PURE__ */ new Date()).toISOString()
        }).select().single();
...
__name(handleConfigSystem, "handleConfigSystem");
async function handleConfigDomainAuth(request) {
  const method = request.method;
  if (method === "GET") {
    try {
      const { data, error } = await supabase.from("domain_authorizations").select("*").order("created_at", { ascending: false });
      if (error)
        throw error;
  ...
  if (method === "POST") {
    try {
      ...
      const { data: existing } = await supabase.from("domain_authorizations").select("id").eq("domain", domain.toLowerCase()).single();
      if (existing) {
        return errorResponse("Domain already authorized", 409);
      }
      const { data, error } = await supabase.from("domain_authorizations").insert({
  ...
  if (method === "PUT") {
    try {
      ...
      const { data, error } = await supabase.from("domain_authorizations").update(updateData).eq("id", id).select().single();
      if (error)
        throw error;
      await supabase.from("operation_logs").insert({
  ...
  if (method === "DELETE") {
    try {
      ...
      const { data: record } = await supabase.from("domain_authorizations").select("*").eq("id", id).single();
      const { error } = await supabase.from("domain_authorizations").delete().eq("id", id);
      if (error)
        throw error;
      await supabase.from("operation_logs").insert({
...
__name(handleConfigDomainAuth, "handleConfigDomainAuth");
async function handleDashboardStats(request) {
  try {
    const [usersCount, cardsCount, rpcCount, logsCount] = await Promise.all([
      ...
      supabase.from("api_logs").select("*", { count: "exact", head: true }).gte("created_at", new Date(Date.now() - 24 * 60 * 60 * 1e3).toISOString())
    ]);
    const rpcHealth = await supabase.from("rpc_nodes").select("health_status").eq("status", true);
    const healthyNodes = rpcHealth.data?.filter((n) => n.health_status === "healthy").length || 0;
    const totalNodes = rpcHealth.data?.length || 0;
    const recentLogins = await supabase.from("operation_logs").select("created_at").eq("action", "LOGIN").order("created_at", { ascending: false }).limit(10);
    const cardStats = await supabase.from("card_keys").select("status");
...
async function checkRPCHealth(url, timeout = 5e3) {
  try {
    const controller = new AbortController();
    const id = setTimeout(() => controller.abort(), timeout);
...
__name(checkRPCHealth, "checkRPCHealth");
async function handleRPCHealth(request) {
  try {
    ...
    for (const node of nodes) {
      ...
      await supabase.from("rpc_nodes").update({
        health_status: healthResult.healthy ? "healthy" : "unhealthy",
        last_health_check: (/* @__PURE__ */ new Date()).toISOString(),
...
__name(handleRPCHealth, "handleRPCHealth");
async function handleRPCList(request) {
  const method = request.method;
  if (method === "GET") {
    try {
      const { data, error } = await supabase.from("rpc_nodes").select("*, rpc_groups(name)").order("priority", { ascending: false });
      if (error) {
  ...
  if (method === "POST") {
    try {
      ...
      const { data, error } = await supabase.from("rpc_nodes").insert({
        name,
        url,
        group_id,
        priority,
...
__name(handleRPCList, "handleRPCList");
var ensCache = /* @__PURE__ */ new Map();
...
__name(handleENSQuery, "handleENSQuery");
...
async function keccak256(data) {
  if (typeof data === "string") {
    const encoder2 = new TextEncoder();
    data = Array.from(encoder2.encode(data));
  }
...
__name(queryENSOwner, "queryENSOwner");
async function POST(request) {
  ...
  if (module === "cards") {
    ...
    if (type === "list")
      return handleCardsList(request);
    if (type === "rules")
      return handleCardsRules(request);
    if (type === "generate")
      return handleCardsGenerate(request);
    if (type === "groups")
      return handleCardsGroups(request);
  }
  if (module === "users") {
    const type = pathParts[pathParts.length - 1];
    if (type === "list")
      return handleUsersList(request);
    if (type === "create")
      return handleUsersCreate(request);
    if (type === "register")
      return handleUsersRegister(request);
    if (type === "send-verify-code")
      return handleUsersSendVerifyCode(request);
    if (type === "groups")
      return handleUsersGroups(request);
  }
  if (module === "config") {
    const type = pathParts[pathParts.length - 1];
    if (type === "site")
      return handleConfigSite(request);
    if (type === "announcement")
      return handleConfigAnnouncement(request);
    if (type === "system")
      return handleConfigSystem(request);
    if (type === "domain-auth")
  ...
  if (module === "rpc") {
    const type = pathParts[pathParts.length - 1];
    if (type === "health")
      return handleRPCHealth(request);
    if (type === "list")
      return handleRPCList(request);
  }
  if (module === "ens") {
...
// .wrangler/tmp/bundle-i5B40G/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-i5B40G/middleware-loader.entry.ts
