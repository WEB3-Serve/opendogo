...
function getSecrets() {
  ...
  const twoFactorCode = process.env.ADMIN_2FA || '';
  const authSecret = process.env.ADMIN_AUTH_SECRET || '';

  if (!account || !password || !twoFactorCode) {
    return {
      ok: false,
  ...
  return {
    ok: true,
    value: { account, password, twoFactorCode, authSecret }
  };
}
...
async function handleLogin(req, res) {
  ...
  const twoFactorOk = safeEqual(twoFactorCode, expected.twoFactorCode);

  if (!accountOk || !passwordOk || !twoFactorOk) {
    return sendJson(res, 401, { success: false, message: 'Invalid account/password/2FA.' });
  }
